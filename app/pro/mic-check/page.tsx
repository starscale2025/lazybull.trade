...
export default function MicCheckPage() {
  ...
  return (
    <div className="min-h-screen bg-bg p-6 font-mono text-fg">
      ...
      <p className="mb-5 text-[11px] text-fg-dim">
      ...
      <section className="mb-5 border border-border bg-bg-soft p-3">
        ...
        <p className="text-[10px] text-fg-faint">Talk — this bar must move. If it doesn&apos;t, it&apos;s an OS/device/permission issue, not the app.</p>
        {devices.length > 0
          ? <div className="mt-2 text-[10px] text-fg-dim">inputs: {devices.join(" · ")}</div>
          : <div className="mt-2 text-[10px] text-bear">no audio input devices visible to the browser — connect a mic/headset, then reload</div>}
      </section>
      ...
      <section className="mb-5 border border-border bg-bg-soft p-3">
        ...
        <div className="mb-3 flex gap-2">
          ...
          <button onClick={stopAll} className="border border-border px-3 py-1.5 text-[11px] uppercase tracking-wider text-fg-dim hover:text-fg">stop</button>
          <span className={`self-center text-[11px] ${running ? "text-bull" : "text-fg-faint"}`}>{running ? "● listening" : "○ idle"}</span>
        </div>
        <div className="mb-2 min-h-[2rem] border border-dashed border-bull/40 p-2 text-[12px] italic text-fg-dim">
          {interim || "(interim transcript appears here as you speak)"}
        </div>
        {finals.length > 0 && (
          <div className="text-[12px]"><span className="text-fg-faint">finals:</span> {finals.map((f, i) => <span key={i} className="mr-2 text-bull">“{f}”</span>)}</div>
        )}
      </section>
