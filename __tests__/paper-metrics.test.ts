...
describe("accountMetrics identities", () => {
  it("reproduces the reference panel's arithmetic", () => {
    // Short 1 AAPL @ 293.32, market 327.22 -> unrealised -33.90, equity 99,966.10
    ...
    // The three published identities must hold exactly.
    expect(m.equity).toBeCloseTo(m.balance + m.unrealizedPnl, 6);
  ...
  it("clamps the buffer to 0..1 rather than showing a negative percentage", () => {
    const over = accountMetrics({
      cash: 1_000,
      realizedToday: 0,
      shares: { A: pos("A", 100, 50) }, // 5000 of margin against 1000 of cash
      orders: [],
      marks: { A: 50 },
    });
