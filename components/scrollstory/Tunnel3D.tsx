...
function Corridor() {
  ...
  const frameMat = useMemo(
    () => new THREE.LineBasicMaterial({ color: "#00ff87", transparent: true, opacity: 0.55 }),
    []
  );
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < mats.length; i++) {
      ...
      if (t && m.map !== t) {
        m.map = t;
        m.color.set("#99ddbb"); // slightly under-white so screens don't clip into bloom
        m.needsUpdate = true;
      }
    }
    // frames breathe faintly — the corridor feels powered-on
    frameMat.opacity = 0.55 + 0.08 * Math.sin(state.clock.elapsedTime * 1.4);
  });
  return (
    <group>
      {PANELS.map((p, i) => (
